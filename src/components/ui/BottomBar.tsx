...
const BottomBar = () => {
  ...
  const DynamicIcon = ({ iconName }: { iconName: string }) => {
    ...
    if (IconComponent) {
      return <IconComponent className="h-[20px] w-[20px]" />;
    }
  ...
  return (
    <div className="fixed bottom-0 h-[60px] w-full flex items-center justify-center bg-gradient-to-b from-white bg-white/95 sm:hidden z-[40]">
        ...
        {sideBarItems.map((item, index) => {
          return (
            <Link href={item.href} key={index} className="">
              <button
                className={`flex flex-col items-center justify-start w-full  ${activeItem === item.href ? " text-red-500  " : ""} hover:opacity-50  rounded-full  px-2 py-[8px] border border-transparent w-fit transition-all duration-200 ease-in-out xl:w-full`}
              >
...
const BottomBarMenu = () => {
  ...
  return (
    <Drawer>
      <DrawerTrigger
        className={`flex flex-col items-center justify-start hover:opacity-50  rounded-full  px-2 py-[8px] border border-transparent w-fit transition-all duration-200 ease-in-out xl:w-full`}
      >
        <LucideIcons.Menu className="h-[20px] w-[20px]" />
